...
print("class(temp_fix$ALT)")
print("class(temp_fix$REF)")
print(class(temp_fix$ALT))
print(class(temp_fix$REF))
temp_fix <- temp_fix %>%
  #dplyr::mutate(ALT=toString(ALT), REF=toString(REF)) %>%
  dplyr::mutate(alt_len=str_length(ALT), ref_len=str_length(REF))
temp_fix <- temp_fix %>% 
...
# locations <- which(str_length(mutations@fix[,'REF']) == 1)
